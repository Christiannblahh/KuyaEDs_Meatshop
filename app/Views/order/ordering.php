<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        <a href="<?= site_url('order/viewOrder') ?>" class="btn btn-primary position-relative">
            <i class="bi bi-cart3"></i> View Order
        </a>
        <a href="<?= site_url('order/orders') ?>" class="btn btn-outline-primary">
                ...
                <div class="product-card card h-100 shadow-sm <?= !$hasStock ? 'opacity-75' : '' ?>">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="mt-auto">
                            ...
                            <?php if ($hasStock): ?>
                                <form class="add-to-order-form" data-product-id="<?= $product['id'] ?>" data-product-name="<?= esc($product['name']) ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    forms.forEach(form => {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('<?= site_url('order/addToOrder') ?>', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    showToast(data.message || 'Failed to add to order', 'error');
                }
            } catch (error) {
